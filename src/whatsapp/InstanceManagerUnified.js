import { WhatsAppInstanceUnified } from './WhatsAppInstanceUnified.js';
import fs from 'fs';
import path from 'path';

export class InstanceManagerUnified {
  constructor() {
    this.instances = new Map();
    this.sessions = new Map();
    this.maxInstances = 10;
    this.cleanupInterval = null;
    this.connectionLock = new Map(); // Lock para evitar conexões simultâneas
  ...
  async createInstance(tenantId, options = {}) {
    try {
      ...
      // Verificar se há lock de conexão para este tenant
      if (this.connectionLock.has(tenantId)) {
        console.log(`⚠️ Conexão já em andamento para tenant: ${tenantId}`);
        return { success: false, error: 'Conexão já em andamento' };
      }

      // Criar lock de conexão
      this.connectionLock.set(tenantId, true);

      try {
        console.log(`🔧 Criando instância para tenant: ${tenantId}`);
        
        const instance = new WhatsAppInstanceUnified(tenantId, options);
        ...
        // Remover lock de conexão
        this.connectionLock.delete(tenantId);
      }
    } catch (error) {
